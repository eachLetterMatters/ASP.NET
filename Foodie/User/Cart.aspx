            ...
            <asp:Repeater ID="repeaterCart" runat="server" OnItemCommand="repeaterCart_ItemCommand" OnItemDataBound="repeaterCart_ItemDataBound">
                ...
                <ItemTemplate>
                    <tr>
                        ...
                        <td>
                            <img width="60" src="<%# Foodie.Utils.GetImageUrl(Eval("image_url")) %>" alt="" />
                        </td>
                        <td>
                            <asp:Label ID="lblPrice" runat="server" Text='<%# Eval("price") %>'></asp:Label> $
                            <asp:HiddenField ID="hdnProductId" runat="server" Value='<%# Eval("product_id") %>'/>
                            <asp:HiddenField ID="hdnQuantity" runat="server" Value='<%# Eval("cart_quantity") %>'/>
                            <asp:HiddenField ID="hdnPrdQuantity" runat="server" Value='<%# Eval("product_quantity") %>'/>
                        </td>
                        <td>
                            <div class="product__details__option">
                                <div class="quantity">
                                    <div class="pro-qty">
                                        <asp:TextBox ID="txtQuantity" runat="server" TextMode="Number" Text='<%# Eval("cart_quantity") %>'>
                                        </asp:TextBox>
                        ...
                        <td>
                            <asp:LinkButton ID="lbDelete" runat="server" Text="Remove" CommandName="remove"  
                                CommandArgument='<%# Eval("product_id") %>' OnClientClick="return confirm('Do you want to remove this item?');">
